...
export default class extends BaseSchema {
  protected tableName = "GIR_GIROS_FUENTE2";

  public async up() {
    this.schema.createTable(this.tableName, (table) => {
      table.comment(
        "Tabla que almacena la carga de información de GIROS de la PRIMERA fuente de información"
      );
      ...
      table.integer("GIR_ID_FONDO", 30).notNullable().comment("Id del Fondo.");
      ...
      table.string("GIR_MODALIDAD", 60).notNullable().comment("Modalidad.");

      table.integer("GIR_PERIODOS").notNullable().comment("Periodos.");
